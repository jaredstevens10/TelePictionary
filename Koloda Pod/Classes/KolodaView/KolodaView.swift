...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    fileprivate func swipedAction(_ direction: SwipeResultDirection) {
        ...
        let shownCardsCount = self.currentCardNumber + self.countOfVisibleCards
        if shownCardsCount - 1 < self.countOfCards {
            ...
            if let dataSource = self.dataSource {
                ...
                var lastCardFrame = self.frameForCardAtIndex(UInt(self.visibleCards.count))
                let cardParameters = self.backgroundCardParametersForFrame(lastCardFrame)
                ...
                if let lastCard = self.visibleCards.last {
                    self.insertSubview(lastCardView, belowSubview:lastCard)
                } else {
                    self.addSubview(lastCardView)
                }
                self.visibleCards.append(lastCardView)
            }
        }
        
        if !self.visibleCards.isEmpty {

            for (index, currentCard) in self.visibleCards.enumerate() {
                currentCard.removeAnimations()
                
                var frameAnimation: POPPropertyAnimation
                var scaleAnimation: POPPropertyAnimation
                if let delegateAnimation = self.delegate?.koloda(kolodaBackgroundCardAnimation: self) {
                    frameAnimation = delegateAnimation.copy() as! POPPropertyAnimation
                    scaleAnimation = delegateAnimation.copy() as! POPPropertyAnimation
                ...
                let cardParameters = self.backgroundCardParametersForFrame(self.frameForCardAtIndex(UInt(index)))
            ...
            self.delegate?.koloda(self, didSwipedCardAtIndex: UInt(self.currentCardNumber - 1), inDirection: direction)
            self.animating = false
            self.delegate?.koloda(kolodaDidRunOutOfCards: self)
        }
