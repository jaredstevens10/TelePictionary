//
//  GalleryItemExtViewController.swift
//  TelePictionary
//
//  Created by Jared Stevens2 on 11/3/16.
...
class GalleryItemExtViewController: UIViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource, MFMessageComposeViewControllerDelegate, UIDocumentInteractionControllerDelegate {
    ...
    //print("THIS GAME INFO: \(ThisGameTurnInfoArray)")
    ...
    */
    ...
    override func viewDidLoad() {
        ...
        */
        ...
        self.refreshControl.addTarget(self, action: #selector(GalleryItemExtViewController.RefreshCommentData(_:)), for: UIControlEvents.valueChanged)
                ...
                DispatchQueue.main.async(execute: {
                   ...
                   // self.actInd.stopAnimating()
                    ...
                    self.backgroundThread(background: {
                        ...
                        print("In background thread now, about to download image")
                        //self.GetPublicTurns()
                        self.DownloadImage()
                    }, completion: {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.playBTN?.addTarget(self, action: #selector(GalleryItemExtViewController.PlayAudioClicked(_:)), for: .touchUpInside)
        // cell.playBTN?.tag = indexPath.row
        
        cell.meBTN?.addTarget(self, action: #selector(GalleryItemExtViewController.ShareImageClicked(_:)), for: .touchUpInside)
        cell.meBTN?.tag = (indexPath as NSIndexPath).row
        cell.friendsBTN?.addTarget(self, action: #selector(GalleryItemExtViewController.NewFailClicked(_:)), for: .touchUpInside)
        cell.friendsBTN?.tag = (indexPath as NSIndexPath).row
        cell.publicBTN?.addTarget(self, action: #selector(GalleryItemExtViewController.NewLikeClicked(_:)), for: .touchUpInside)
        cell.publicBTN?.tag = (indexPath as NSIndexPath).row
        cell.bestfriendsBTN?.addTarget(self, action: #selector(GalleryItemExtViewController.ViewCommentsClicked(_:)), for: .touchUpInside)
                        ...
                        DispatchQueue.main.async(execute: {
                            if let cellToUpdate = collectionView.cellForItem(at: indexPath) {
                                ...
                                if MyTurnData.mediaType == "gif" {
                                    //TEMP COMMENTOUT
                                    //cell.TurnImage?.image = UIImage.animatedImagewithanimated
                                    cell.TurnImage?.image = UIImage.animatedImage(withAnimatedGIFData: data!)
                                    
                                    print("Turn was a GIF: \(MyTurnData.TurnNumber)")
                                   // cell.TurnImage?.image = UIImage.an
                                } else {
    ...
    func DownloadImage() {
       ...
       // print("Downloading image now, ThisGameTurnInfoArray: \(ThisGameTurnInfoArray)")
    ...
    func GetCompleteGameData () {
        if Reachability.isConnectedToNetwork() {
            ...
            //print("GETTING COMPLETE GAME DATA")
            ...
            let URLData = GetGalleryGameData(TempGameID)
            ...
            let CompleteGameInfoArray = FilterGameDataGallery(URLData)
                   ...
                   // print("THIS GAME COMPLETE GAME INFO ARRAY = \(CompleteGameInfoArray)")
    ...
    func fileExists(_ url : URL!) -> Bool {
        ...
        print("FILE EXISTS RESPONSE: \(response)")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.TurnByHolder.layer.cornerRadius = 10
        cell.TurnByHolder.clipsToBounds = true
        cell.TurnByHolder.layer.masksToBounds = true
        ...
        cell.ShareButton?.addTarget(self, action: #selector(GalleryItemExtViewController.ShareImageClicked(_:)), for: .touchUpInside)
        cell.ShareButton?.tag = (indexPath as NSIndexPath).row
        cell.FailButton?.addTarget(self, action: #selector(GalleryItemExtViewController.NewFailClicked(_:)), for: .touchUpInside)
        cell.FailButton?.tag = (indexPath as NSIndexPath).row
        cell.LikeButton?.addTarget(self, action: #selector(GalleryItemExtViewController.NewLikeClicked(_:)), for: .touchUpInside)
        cell.LikeButton?.tag = (indexPath as NSIndexPath).row
        cell.commentsBTN?.addTarget(self, action: #selector(GalleryItemExtViewController.ViewCommentsClicked(_:)), for: .touchUpInside)
